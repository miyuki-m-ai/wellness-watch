...
def speech_to_text(audio_path: str) -> str:
    ...
    if result.reason == speechsdk.ResultReason.RecognizedSpeech:
        logging.info("🎤 STT認識完了")
        return result.text.strip()

    if result.reason == speechsdk.ResultReason.Canceled:
        details = speechsdk.CancellationDetails.from_result(result)
        logging.warning(f"⚠️ STTキャンセル：{details.reason} / {details.error_details}")
    else:
        logging.warning(f"⚠️ STT失敗：{result.reason}")
...
def text_to_speech(text: str, output_path: str) -> bool:
    ...
    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        logging.info(f"🔊 TTS成功：{output_path}")
        return True

    if result.reason == speechsdk.ResultReason.Canceled:
        details = speechsdk.CancellationDetails.from_result(result)
        logging.warning(f"⚠️ TTSキャンセル：{details.reason} / {details.error_details}")
    else:
        logging.warning(f"⚠️ TTS失敗：{result.reason}")
...
def get_line_audio_content(message_id: str) -> bytes | None:
    ...
    try:
        ...
        logging.warning(f"⚠️ 音声コンテンツ取得失敗：{e}")
        return None
...
def get_line_audio_content_dad(message_id: str) -> bytes | None:
    ...
    try:
        ...
        logging.warning(f"⚠️ お父さん音声コンテンツ取得失敗：{e}")
        return None
...
def upload_and_reply_audio(reply_token: str, wav_path: str, user_id: str):
    ...
    try:
        ...
        logging.info(f"☁️ Blob アップロード成功：{blob_name}")
    except Exception as e:
        logging.warning(f"⚠️ Blob アップロード失敗：{e}")
        return
    ...
    try:
        ...
        logging.info("🔗 SAS URL 生成成功")
    except Exception as e:
        logging.warning(f"⚠️ SAS URL 生成失敗：{e}")
        return
    ...
    try:
        ...
        logging.info(f"⏱️ 音声長さ：{duration_ms}ms")
    except Exception as e:
        logging.warning(f"⚠️ 音声長さ取得失敗（デフォルト5秒使用）：{e}")
    ...
    try:
        ...
        logging.info(f"🔊 音声返信成功（{duration_ms}ms）")
    except urllib.error.HTTPError as e:
        error_body = e.read().decode("utf-8")
        logging.error(f"❌ 音声返信失敗：{e.code} {e.reason}")
        logging.error(f"   詳細：{error_body}")
        logging.error(f"   URL：{audio_url[:80]}...")
    except Exception as e:
        logging.error(f"❌ 音声返信失敗：{e}")
...
def reply_text(reply_token: str, message: str):
    ...
    try:
        urllib.request.urlopen(req)
        logging.info("✅ テキスト返信成功")
    except Exception as e:
        logging.error(f"❌ テキスト返信失敗：{e}")
...
def verify_signature_dad(body: bytes, signature: str) -> bool:
    hash_ = hmac.new(
...
def handle_message(user_id: str, reply_token: str, user_text: str, use_voice: bool = False):
    ...
    if user_text.strip() in ["レポート", "report", "報告"]:
        ...
        valid_scores = [d["avg_sentiment"] for d in days if d.get("avg_sentiment") is not None]
    ...
    logging.info(
        f"  └ 感情：{result.get('sentiment', 0):.2f} "
...
# 共通イベント処理
# =============================================
def process_event(event: dict, use_dad_audio: bool = False):
    ...
    logging.info(f"📩 受信：{user_id} | タイプ：{msg_type}")
    ...
    if msg_type == "audio":
        ...
        audio_bytes = (
            get_line_audio_content_dad(message_id)
            if use_dad_audio
            else get_line_audio_content(message_id)
        )
        ...
        try:
            ...
            logging.info(f"🔄 m4a→wav変換成功：{wav_path_stt}")

        except Exception as e:
            logging.warning(f"⚠️ 音声変換失敗：{e}")
        ...
        logging.info(f"📝 STT完了（文字数：{len(recognized_text)}文字）")
        handle_message(user_id, reply_token, recognized_text, use_voice=True)
        return

    logging.info(f"（未対応のメッセージタイプ：{msg_type}）")
...
def callback():
    ...
    logging.info("🔥 イベント受信前")

    signature = request.headers.get("X-Line-Signature", "")
    body = request.get_data()

    if not verify_signature(body, signature):
        logging.error("❌ 署名検証失敗")
        abort(400)

    events = json.loads(body).get("events", [])
    logging.info("🔥 events取得")

    for event in events:
        process_event(event, use_dad_audio=False)
...
def callback_dad():
    logging.info("🔥 callback_dad start")

    signature = request.headers.get("X-Line-Signature", "")
    body = request.get_data()

    if not verify_signature_dad(body, signature):
        logging.error("❌ お父さん用署名検証失敗")
        abort(400)

    events = json.loads(body).get("events", [])

    for event in events:
        process_event(event, use_dad_audio=True)
...
if __name__ == "__main__":
    ...
    logging.info("=== LINE Bot サーバー起動（音声対応版）===")
    logging.info("ポート：5000")
